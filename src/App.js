...
class BooksApp extends React.Component {
  ...
  sortBookListing = (books) => {
    ...
    SHELF_TYPES.forEach(type => {
        let bookGrouping = books.filter(book => {
  ...
  render() {
    ...
    types.forEach(type => {
        ...
        shelves.push(<Row title={getShelfTypeName(type)} key={`shelf-${type}`}>{booksRender}</Row>); 
    });
    ...
    return (
      <div className="app">
          <Route path="/search" render={()=>{
              return <Search onShelfChanged={this.onShelfChanged}/>
          }
          }/>
